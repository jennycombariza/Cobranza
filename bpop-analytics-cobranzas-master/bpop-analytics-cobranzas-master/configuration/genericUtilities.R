...
setWorkspace <- function(rootPath = "N:/_Projects/201802_ZZZ000_GrupoAval_ABIERTO/"){
  
  # Creates all the working directories used to reference file locations
  cat("Creando rutas del WorkSpace", fill = TRUE)

  basePath <<- osPathJoin(rootPath)
  
  # Data paths
  # Estas rutas hacen referencia al bucket de S3
  dataPath <<- osPathJoin("/1.Data")
  originalDataPath <<- osPathJoin(dataPath, "RawData")
  processedDataPath <<- osPathJoin(dataPath, "ProcessedData")
  expandedDataPath <<- osPathJoin(dataPath, "ExpandedData")
  
  
  # File directories of every raw table
  moraRawPath <<- osPathJoin(originalDataPath, "mora")
  buroRawPath <<- osPathJoin(originalDataPath, "buro")
  contratosRawPath <<- osPathJoin(originalDataPath, "contratos")
  txPasivoRawPath <<- osPathJoin(originalDataPath, "tx_pasivo")
  clientesRawPath <<- osPathJoin(originalDataPath, "clientes")
  
  # File directories of every processed table
  moraProcessedPath <<- osPathJoin(processedDataPath, "mora")
  buroProcessedPath <<- osPathJoin(processedDataPath, "buro")
  contratosProcessedPath <<- osPathJoin(processedDataPath, "contratos")
  txPasivoProcessedPath <<- osPathJoin(processedDataPath, "tx_pasivo")
  clientesProcessedPath <<- osPathJoin(processedDataPath, "clientes")
  
  # File directories of every processed table
  customerBaseExpandedPath <<- osPathJoin(expandedDataPath, "CustomerBase")
  targetExpandedPath <<- osPathJoin(expandedDataPath, "Target")
  moraExpandedPath <<- osPathJoin(expandedDataPath, "mora")
  buroExpandedPath <<- osPathJoin(expandedDataPath, "buro")
  contratosExpandedPath <<- osPathJoin(expandedDataPath, "contratos")
  txPasivoExpandedPath <<- osPathJoin(expandedDataPath, "tx_pasivo")
  masterTableExpandedPath <<- osPathJoin(expandedDataPath, "masterTable")
  
  # Validacion qualitativa de los ficheros Raw
  dataQualityAnalysisPath <<- osPathJoin(dataPath, "InputValidation")
  
  # Dataset utilizado para el entrenamiento, agregacion de los ficheros masterTable
  datasetPreparedPath <<- osPathJoin(dataPath, "Dataset")
  oldDatasetPreparedPath <<- osPathJoin(dataPath, "DatasetRepositorio")
  
  
  outputsPath <<- osPathJoin("/0.Outputs")
  
  
  # Carpeta que contiene los modelos entrenados y los cortes para definir el riesgo
  genericModelPath <<- osPathJoin(outputsPath, "Modelo")
  modelsPath <<- osPathJoin(genericModelPath, "ModelosXGBoost")
  boundsLogExposurePredPath <<- osPathJoin(genericModelPath, "CortesLogExposureScore")
  variablesPath <<- osPathJoin(genericModelPath, "Variables")
  modelosRepositorioPath <<- osPathJoin(genericModelPath, "Repositorio")
  oldModelsPath <<- osPathJoin(modelosRepositorioPath, "RepositorioModelos")
  oldBoundsLogExposurePredPath <<- osPathJoin(modelosRepositorioPath, "RepositorioCortesLogExposureScore")
  oldVariablesPath <<- osPathJoin(modelosRepositorioPath, "VariablesRepositorio")
  
  
  # Carpeta con las predicciones diarias 
  predictionsPath <<- osPathJoin(outputsPath, "Predicciones")
  dailyClientVisionPath <<- osPathJoin(predictionsPath, "VisionCliente")
  dailyProductVisionPath <<- osPathJoin(predictionsPath, "VisionProducto")
  dailyPredBreakdownPath <<- osPathJoin(predictionsPath, "PredBreakdown") # Solo Popular
  
  # Carpeta con los resultados y performance de los modelos entrenados
  
  trainingPath <<- osPathJoin(outputsPath, "Entrenamiento")
  modelValidationPath <<- osPathJoin(trainingPath, "ModelValidation")
  idScorePath <<- osPathJoin(modelValidationPath, "Predicciones")
  trainingClientVisionPath <<- osPathJoin(idScorePath, "VisionCliente")
  trainingProductVisionPath <<- osPathJoin(idScorePath, "VisionProducto")
  predBreakdownPath <<- osPathJoin(modelValidationPath, "PredBreakdown")
  modelPerformanceByPercentilePath <<- osPathJoin(modelValidationPath, "ModelPerformanceByPercentile")
  trainingVariablesAnalysisPath <<- osPathJoin(modelValidationPath, "Variables")
  varImpPath <<- osPathJoin(trainingVariablesAnalysisPath, "VariablesImportantes")
  variableSummaryByTranchePath <<- osPathJoin(trainingVariablesAnalysisPath, "VariableSummaryByTranche")
  rocCurvePath <<- osPathJoin(modelValidationPath, "ROCCurve")
  
  # Carpeta con los resultados de los backtestin realizados
  
  backtestingPath <<- osPathJoin(outputsPath, "Backtesting")
  backtestingModelValidationPath <<- osPathJoin(backtestingPath, "ModelValidation")
  backtetingIDScorePath <<- osPathJoin(backtestingModelValidationPath, "Predicciones")
  backtetingClientVisionPath <<- osPathJoin(backtetingIDScorePath, "VisionCliente")
  backtetingProductVisionPath <<- osPathJoin(backtetingIDScorePath, "VisionProducto")
  backtetingPredBreakdownPath <<- osPathJoin(backtestingModelValidationPath, "PredBreakdown")
  backtetingModelPerformanceByPercentilePath <<- osPathJoin(backtestingModelValidationPath, "ModelPerformanceByPercentile")
  backtestinVariableSummaryByTranchePath <<- osPathJoin(backtestingVariablesAnalysisPath, "VariableSummaryByTranche")
  backtestingRocCurvePath <<- osPathJoin(backtestingModelValidationPath, "ROCCurve")
...
loadProjectFunctions <- function(){
  ...
  # into a prepared monthly table
  cat("Loading auxiliary functions", fill = TRUE)
  files <- list.files(projectFunctionsPath, full.names = T, pattern = ".R")
  for(script in files){
    cat("Loading", script, fill = TRUE)
    source(script)
    }
...
librariesRequired <- c("bit64", "data.table", "tidyr", "stringr", 
                       ...
                       # "xgboostExplainer",
                       "tictoc", "ROCR", "png", "plotly", "htmlwidgets",
                       "purrr", "rjson"
                       )
...
ModelPerformanceByPercentileCollections <- function(dir = "~", validationSet, target,
  ...
  fwrite_s3(dt, filePath = file.path(dir, paste0("Uplift_", title, ".csv")), row.names = FALSE, sep = sep, dec = dec)
...
uplift_evalxgb <- function(preds, dtrain) {
  ...
  topPercent <- results[1:(nrow(results)*uplift_evalxgb_quantileGlobal), ]
...
saveProcessedTableByPeriod <- function(dt, 
                                       path, 
                                       source, 
                                       periodIDvarname = "YEAR_MONTH"){
  for(mon in unique(dt[, get(periodIDvarname)])){
    outputFilename <- source %+% "_" %+% mon %+% ".csv"
    cat("Guardando", outputFilename, "en el bucket", auxBucket, fill = TRUE)
    fwrite_s3(file = dt[get(periodIDvarname) == mon], filePath = osPathJoin(path, outputFilename))
  }
}

getProcessedTableByPeriod <- function(dataSource, periods, path, extension = ".csv", ...){
  if(length(periods) == 0){
    fileNames <- c()
  } else {
    fileNames <- osPathJoin(path, dataSource %+% "_" %+% periods %+% extension)
  }
  
  if(length(periods) == 1){
    dt <- fread_s3(fileNames[1], ...)  
  } else {
    dt <- fread_s3_parallel(fileNames, ...)
  }
  return(invisible(dt))
}
...
saveTableSummariesByPeriod <- function(dt, path = dataQualityAnalysisPath, source, periodIDvarname = "YEAR_MONTH"){
  for(mon in unique(dt[, get(periodIDvarname)])){
    outputFilename <- dataQualityAnalysisPath %+% "/Validacion_" %+% source %+% "_" %+% mon %+% ".csv"
    SummariseTable(dt[get(periodIDvarname) == mon], filename = outputFilename)
  }
}

#' Calculate most important model metrics performance 
#' @param dir string that gives the path where the final csv file will be saved.
#' @param validationSet data.frame or data.table object with the target
#'   variable and the associated scores (scores are optional).
#' @param target a string with the name of the column target variable inside 
...
#'
#' @examples
#' ModelPerformanceByPercentile(dir = "Reports/", validationSet = test, 
#' target = "TARGET", scores = "PREDICTION", step = 0.1, title = "Example")
#' @export

ModelPerformanceByPercentile <- function(dir = "~", validationSet, target, 
                                         scores, step = 0.1, title = "test",
                                         sep = ";", dec = ","){
  ...
  if (class(scores) == "character"){
    scoPos <- which(names(validationSet) == scores)
    dt <- data.table(predicted = validationSet[[scoPos]], target = validationSet[[tarPos]])
  } else if (class(scores) == "numeric") {
    dt <- data.table(predicted = scores, target = validationSet[[tarPos]])
  } else {
    stop(paste0("Cannot recognise the class of the argument scores.",
                "Currently only character and numeric supported."))
  }
...
createVariableSummaryPerScoreTranche <- function(dt, OutputVarImp, outputName, outputDir = NA, predcolName = "predxgb"){
  scores <- copy(dt)
  setorderv(scores, predcolName, order = -1)
  scores[, tranche := seq(1, .N)/.N*100]
  scores[, tranche := cut(tranche, c(0:10)*10,
                          paste0(c(1:10)*10, "%"))]
  varsToNumeric <- OutputVarImp[1:50][,Feature]
  
  for(var in varsToNumeric){
    set(x = scores,
        j = var,
        value = scores[[var]] %>% as.numeric)
  }
  
  for(var in varsToNumeric){
    set(x = scores,
        j = var,
        i = scores[, which(is.infinite(get(var)))],
        value = NA)
  }
  
  ratios <- varsToNumeric %gv% "ratio|RATIO"
  non_ratios <- setdiff(varsToNumeric, ratios)
  auxPercentage <- scores[, as.numeric(.N), by = .(tranche)]
  auxPercentage[, V1 := V1/sum(V1)]
  results_non_ratios <- scores[, lapply(.SD, function(x) mean(as.numeric(x), na.rm = TRUE)),
                               .SDcols = non_ratios,
                               by = tranche]
  
  results_ratios <- scores[, lapply(.SD, function(x) median(as.numeric(x), na.rm = TRUE)),
                           .SDcols = ratios,
                           by = tranche]
  results <- cbind(results_ratios, results_non_ratios[, -c("tranche")])
  results <- merge(results, auxPercentage,
                   by = "tranche")
  
  results <- dcast.data.table(melt(results, id.vars = "tranche"), variable ~ tranche)
  results[, metricUsed := ifelse(variable %in% ratios, "Median", "Mean")]
  results <- merge(results, OutputVarImp, by.x = c("variable"), by.y = c("Feature"), all.x = T)
  setorder(results, -Gain)
  
  if(!is.na(outputDir)){
    fwrite_s3(results, filePath = paste0(outputDir, "/", outputName, ".csv"),
              sep = ";", dec = ".")
  }
  else{
    filename <- paste0(dirs$Outputs, outputName, sep = "/")
    fwrite_s3(results, filePath = paste0(outputDir, "/", outputName, ".csv"),
              sep = ";", dec = ".")
  }
...
DrawROCCurve <- function(dir = NULL, validationSet, target, scores, colours = c("#EC0D06", "#000000"), 
  ...
  if (is.null(dir)){
    ...
    fileName <- paste0("ROC_model_", title, ".png")
    ggsave(plot = p, filename = file.path(rootPath, fileName), ...)
    system(paste0("aws s3 cp ", file.path(rootPath, fileName), " ",
                  get_key_s3(osPathJoin(dir, fileName), bucket = auxBucket)))
  }
}
...
# formatAsNumeric replaces commas with dots and transforms to numeric. this function is vectorized
formatAsNumeric <- purrr::compose(
  as.numeric,
  purrr::partial(gsub, pattern = ',', replacement = '.')
)


# formatAsString removes leading and trailing white spaces, makes all characters uppercase and changes encoding (removing accents).
# this function is vectorized
formatAsString <- purrr::compose(
  purrr::partial(iconv, from = 'UTF-8', to = 'ASCII//TRANSLIT'),
  toupper,
  purrr::partial(stringr::str_trim, side = 'both')
)



# Logica del dia a seleccionar
crearPeriodoASeleccionar <- function(periodo){
  lagData <- 10
  periodoMes <- str_sub(periodo, 1, 6) %>% as.numeric
  periodoMesBuro <- ifelse(as.numeric(periodoMes) %% 2 == 0, 
                           getPastPeriod(periodoMes, 3),
                           getPastPeriod(periodoMes, 2))
  
  if(day(ymd(periodo)) > lagData){
    periodoMes <- monthAddInteger(periodoMes, -1)
  } else {
    periodoMes <- monthAddInteger(periodoMes, -2)
  }
  return(list(periodo, periodoMes, periodoMesBuro))  
}
